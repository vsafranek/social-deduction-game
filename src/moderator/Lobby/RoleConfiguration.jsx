...
function RoleConfiguration({
  ...
  const [showGuaranteedModal, setShowGuaranteedModal] = useState({
    team: null,
    show: false,
  });
  ...
  const clamp = (v) =>
    Math.max(10, Math.min(1800, Number.isFinite(+v) ? +v : 10));
  ...
  const guaranteedByTeam = guaranteedRoles.reduce(
    (acc, role) => {
  ...
  const distributedRolesByTeam = roleKeys.reduce(
    (acc, r) => {
      const t = teamOf(r);
  ...
  const teamPoolSizeByTeam = roleKeys.reduce(
    (acc, r) => {
      if (randomPoolRoles[r]) {
        const team = teamOf(r);
  ...
  const actualRolesByTeam = {
    ...
    neutral:
      (distributedRolesByTeam.neutral || 0) + (guaranteedByTeam.neutral || 0),
  };
      ...
      <div className="roles-section">
          ...
          <div className="timer-sliders">
            <div className="timer-slider-item">
              ...
              <input
                ...
                value={nightSeconds}
                onChange={(e) =>
                  pushTimers("night", parseInt(e.target.value, 10))
                }
              />
            </div>
            <div className="timer-slider-item">
              ...
              <input
                ...
                value={daySeconds}
                onChange={(e) =>
                  pushTimers("day", parseInt(e.target.value, 10))
                }
              />
            </div>
          </div>
        ...
        <div className="team-limits-section">
          <h3>
            ⚖️ Team Limits
            <span
              className={`roles-count-info ${
                rolesMatchPlayers ? "success" : "warning"
              }`}
            >
          ...
          {showGuaranteedModal.show &&
            showGuaranteedModal.team &&
            (() => {
              ...
              return (
                <div
                  className="guaranteed-modal-overlay"
                  onClick={handleCloseGuaranteedModal}
                >
                  <div
                    className="guaranteed-modal"
                    ...
                    <div className="guaranteed-modal-body">
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              const result = canAddRandomRole({
                                ...
                                teamGuaranteedCount,
                                teamPoolSize:
                                  teamPoolSizeByTeam[currentTeam] || 0,
                              });
                              if (result.canAdd) {
                                updateTeamLimit(currentTeam, currentLimit + 1);
                              }
                      ...
                      <div className="guaranteed-section-modal">
                        ...
                        {roleKeys
                          .filter(
                            (r) =>
                              teamOf(r) === currentTeam && randomPoolRoles[r]
                          )
                          .map((role) => {
                            const guaranteedCount = guaranteedRoles.filter(
                              (r) => r === role
                            ).length;
                            ...
                            const isUnlimited =
                              maxLimit === null || maxLimit === undefined; // null = unlimited
                            const displayLimit = isUnlimited
                              ? poolCount
                              : maxLimit;
                            const maxLimitText = isUnlimited
                              ? "∞"
                              : displayLimit;
                            ...
                            return (
                              <div
                                key={role}
                                className="guaranteed-role-item-modal"
                              >
                                ...
                                <div className="guaranteed-role-controls-modal">
                                  ...
                                  <span className="guaranteed-count-modal">
                                    {guaranteedCount}
                                  </span>
                                  <button
                                    className="counter-btn plus"
        ...
        <button
          className="btn-open-role-config"
