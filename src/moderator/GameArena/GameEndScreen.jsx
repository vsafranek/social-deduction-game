...
const WINNER_LABELS = {
  good: {
    ...
    gradient:
      "linear-gradient(135deg, rgba(46, 125, 50, 0.3), rgba(27, 94, 32, 0.2))",
    description: "All criminals have been eliminated!",
  },
  evil: {
    label: "The Shadows win!",
    gradient:
      "linear-gradient(135deg, rgba(198, 40, 40, 0.3), rgba(183, 28, 28, 0.2))",
    description: "The Shadows have taken control!",
  },
  solo: {
    label: "Solo Victory!",
    gradient:
      "linear-gradient(135deg, rgba(245, 127, 23, 0.3), rgba(230, 81, 0, 0.2))",
    description: "Last survivor!",
  },
  custom: {
    label: "Special Victory!",
    gradient:
      "linear-gradient(135deg, rgba(106, 27, 154, 0.3), rgba(74, 20, 140, 0.2))",
    description: "Special win condition!",
  },
  unknown: {
    label: "Unknown Result",
    gradient:
      "linear-gradient(135deg, rgba(69, 90, 100, 0.3), rgba(38, 50, 56, 0.2))",
...
function GameEndScreen({
  ...
  if (winner === "custom") {
    const winnerPlayer = players.find((p) =>
      winnerIds.includes(p._id?.toString?.() ?? p._id)
    );
    if (winnerPlayer?.role === "Jester") {
      winnerInfo = {
        label: "Jester Victory!",
        gradient:
          "linear-gradient(135deg, rgba(123, 31, 162, 0.3), rgba(74, 20, 140, 0.2))",
      ...
      winnerInfo = {
        ...
        gradient:
          "linear-gradient(135deg, rgba(81, 45, 168, 0.3), rgba(49, 27, 146, 0.2))",
  ...
  const isCurrentPlayer = (playerId) => {
    return (
      currentPlayer && playerId.toString() === currentPlayer._id.toString()
    );
  };
  ...
  const renderPlayerCard = (player, showTeamLabel = true) => {
    ...
    // Get all modifiers from history
    const modifierHistory = player.roleData?.modifierHistory || [];
    const currentModifier = player.modifier;
    // Get unique modifiers (from history + current)
    const allModifiers =
      modifierHistory.length > 0
        ? [
            ...new Set(
              [
                ...modifierHistory.map((m) => m.modifier),
                currentModifier,
              ].filter(Boolean)
            ),
          ]
    ...
    return (
      <div
        key={player._id}
        className={`player-card ${isPlayerWinner ? "winner" : "loser"} ${
          isCurrent ? "self" : ""
        } ${isDead ? "dead" : ""}`}
      >
        {/* Player name with avatar and crown at the top */}
        <div className="player-card-name-header">
          {hasAvatar ? (
            <img
          ...
          <div
            ...
            style={{
              ...
              background:
                "radial-gradient(circle at 30% 30%, #3e2723, #1a1520)",
          ...
          {isPlayerWinner && (
            <span className="winner-crown" title="Winner">
              👑
            </span>
          )}
        </div>
        ...
        <div className="player-card-body">
          ...
          {/* Display all modifiers from history */}
          {allModifiers.length > 0 && (
            <div className="modifiers-list">
              {allModifiers.map((modifier, idx) => {
                const modInfo = getModifierInfo(modifier);
                if (!modInfo) return null;
                const isCurrent = modifier === currentModifier;
                return (
                  <span
                    key={`${modifier}-${idx}`}
                    ...
                    title={
                      isCurrent
                        ? modInfo.label
                        : `${modInfo.label} (had during game)`
                    }
                  >
                    <RoleIcon
                      role={modifier}
                      size={28}
                      className="modifier-icon-inline"
                      isModifier={true}
                    />
                    {modInfo.label}
                    {!isCurrent && (
                      <span
                        className="modifier-history-indicator"
                        title="Had during game"
                      >
                        *
                      </span>
                    )}
    ...
    <div className="game-end-screen">
      ...
      {showAnimation && (
        <div
          ...
          style={{ background: winnerInfo.gradient }}
        >
      ...
      <div className="end-content">
        ...
        <div className="teams-container">
          ...
          {/* Evil and Neutral Teams Side by Side */}
          {(evilPlayers.length > 0 || neutralPlayers.length > 0) && (
            <div className="evil-neutral-row">
              {/* Evil Team */}
