...
function NightResults({ player, results = [] }) {
  ...
  const formatDetail = (result) => {
    ...
    if (result.type === "visited" && !canSeeVisitorNames) {
      ...
      if (count === 1) {
        return "Někdo tě navštívil.";
      }

      return `${count} hráčů tě navštívilo.`;
    }
  ...
  const mainEventData =
    RESULT_MAPPING[mainResult.type] || RESULT_MAPPING["safe"];
      ...
      >
        <div className="result-summary-content">
          ...
          <div className="result-text">
            ...
            {mainDetail && <span className="result-detail">{mainDetail}</span>}
          </div>
          {shouldShowExpand && (
      ...
      {expanded && displayedResults.length > 1 && (
        <div className="night-results-details">
          {displayedResults.map((result, idx) => {
            const eventData =
              RESULT_MAPPING[result.type] || RESULT_MAPPING["safe"];
            ...
            return (
              ...
              >
                ...
                <div className="result-item-text">
                  ...
                  {formatDetail(result) && (
                    <span className="result-detail-small">
                      {formatDetail(result)}
                    </span>
                  )}
                </div>
